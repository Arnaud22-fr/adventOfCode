...
public class Carte implements Graphe {
	...
	public ArrayList<Noeud> listeNoeudsAdjacents(Noeud source) {
		...
		// Voisin de droite
		if (colonne - 1 >= 0 && tuiles[ligne][colonne - 1].Accessible((Tuile) source)) {
			listeNoeudsSortants.add(tuiles[ligne][colonne - 1]);
		}

		// Voisin de gauche
		if (colonne + 1 < nbColonnes && tuiles[ligne][colonne + 1].Accessible((Tuile) source)) {
			listeNoeudsSortants.add(tuiles[ligne][colonne + 1]);
		}

		// Voisin du haut
		if (ligne - 1 >= 0 && tuiles[ligne - 1][colonne].Accessible((Tuile) source)) {
			listeNoeudsSortants.add(tuiles[ligne - 1][colonne]);
		}

		// Voisin du bas
		if (ligne + 1 < nbLignes && tuiles[ligne + 1][colonne].Accessible((Tuile) source)) {
			listeNoeudsSortants.add(tuiles[ligne + 1][colonne]);
		}
	...
	public ArrayList<Arc> listeArcsSortants(Noeud source) {
		...
		if (tuiles[ligne][colonne].Accessible((Tuile) source)) {
			// Droite
			if (colonne - 1 >= 0 && tuiles[ligne][colonne - 1].Accessible((Tuile) source)) {
	...
	public String reconstruireChemin() {
		...
		System.out.println("nb case parcourues : " + nbVisite);
		chemin = noeudCourant.toString() + chemin;
		return chemin;
	}
	...
	public void effacer() {
		...
		// Effacer les distances et pr�curseurs
		for (int ligne = 0; ligne < nbLignes; ligne++) {
			for (int colonne = 0; colonne < nbColonnes; colonne++) {
