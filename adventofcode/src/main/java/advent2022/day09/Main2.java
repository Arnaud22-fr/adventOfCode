...
public class Main2 {
	...
	public static void main(String[] args) {
		...
		List<Pair<Integer, Integer>> knots = Arrays.asList(Pair.of(i, j), Pair.of(i, j), Pair.of(i, j), Pair.of(i, j),
				Pair.of(i, j), Pair.of(i, j), Pair.of(i, j), Pair.of(i, j), Pair.of(i, j), Pair.of(i, j));
		...
		for (String deplacement : deplacements) {
			...
			for (int pas = 0; pas < distance; pas++) {
				...
				knots.set(0, posHead);
				for (int s = 1; s < knots.size(); s++) {
					posTail = getNewPositionTail(direction, knots.get(s - 1), knots.get(s));
					knots.set(s, posTail);
